...
3. `modules/` - stores modules for reusable code.
...
6. `vars-dev.tfvars` and `vars-test.tfvars` - set the values for those variables.
...
3. To confirm that the configuration behaves as expected, run the following commands in the terminal:
    ...
    3. `terraform plan -var-file="./vars-dev.tfvars"`
        ...
        `-var-file=""` sets which `.tfvars` file to utilise for variable values.
